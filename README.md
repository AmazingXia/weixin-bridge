...
Standalone Weixin long-poll bridge. It does not require OpenClaw. It supports QR login, long-poll inbound messages, sending replies, a local chat test command, and custom realtime integrations through Node API or HTTP webhook.
...
A QR code will appear in the terminal. Scan it with Weixin and confirm on the phone. Credentials are stored under `~/.weixin-bridge/accounts` by default. Override with `WEIXIN_STATE_DIR`.
...
Send a message to the logged-in Weixin account. The bridge will reply with `收到：<your message>`.
...
The bridge communicates with the Weixin iLink backend via HTTP JSON API. Developers integrating with their own backend need to implement the following interfaces.
